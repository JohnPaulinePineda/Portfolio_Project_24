...
###  1.5.16 Threshold Criterion Using Cost-Benefit Methodology (TC_CB)
|
| [Cost-Benefit Methodology](https://www.nejm.org/doi/full/10.1056/NEJM197507312930501) is a threshold criterion based on cost-benefit methodology by means of calculating the slope of the ROC curve at the optimal cutoff. This method weighs the relative costs of the different predictions in the diagnosis.
|
| **[A]** The <span style="color: #0000FF">CB</span> threshold criterion from the <mark style="background-color: #CCECFF">**OptimalCutpoints**</mark> package was used which weighed the relative costs of the different predictions in the diagnosis.
|  
| **[B]** The threshold criterion requires to set 1 parameter:
|      **[B.1]** <span style="color: #FF0000">costs.ratio</span> = ratio between two computations: cost difference of the false positive and true negative cases; and the cost difference of the false negative and true positive cases, set at 2.0
| 
| **[C]** The determined optimal dichotomization threshold and corresponding classification metrics are summarized as follows:
|      **[C.1]** Log Solubility Class Probability Threshold = 0.64163
|      **[C.2]** Apparent Sensitivity = 0.90458
...
######################################
ClassificationMetrics.Class0.LO.CB <- function(SourceData,Response,Predictor){
  ThresholdCriterion <- optimal.cutpoints(X = Predictor,
                                         status = Response,
                                         tag.healthy = "Low",
                                         methods = c("CB"),
                                         data = SourceData,
                                         pop.prev = NULL,
                                         control = control.cutpoints( costs.ratio=2),
                                         ci.fit = FALSE,
                                         direction = c("<"))
  
  summary(ThresholdCriterion)
  (OptimalThreshold <- ThresholdCriterion$CB$Global$optimal.cutoff$cutoff[1])
  (ThresholdSensitivity <- ThresholdCriterion$CB$Global$optimal.cutoff$Se[,1][1])
  (ThresholdSpecificity <- ThresholdCriterion$CB$Global$optimal.cutoff$Sp[,1][1])
  (ThresholdROCAUC <- ThresholdCriterion$CB$Global$measures.acc$AUC[1])
  plot.optimal.cutpoints(ThresholdCriterion, 
                         col=c("#5680E9"), 
                         legend=FALSE, 
                         which=1, 
...
ClassificationMetrics.Class0.HI.CB <- function(SourceData,Response,Predictor){
  ThresholdCriterion <- optimal.cutpoints(X = Predictor,
                                         status = Response,
                                         tag.healthy = "Low",
                                         methods = c("CB"),
...
for (i in 7:7) {
  ...
  ClassificationMetrics.i <- ClassificationMetrics.Class0.LO.CB(LR_Model_Predictions,
                                                                    Response.i,
                                                                    Predictor.i)
  OPTTH.i <- as.numeric(as.character(sprintf(ClassificationMetrics.i[1],fmt='%#.5f')))
  THSEN.i <- as.numeric(as.character(sprintf(ClassificationMetrics.i[2],fmt='%#.5f')))
...
CB.SUMMARY <- as.data.frame(ClassificationMetrics.SUMMARY)
CB.SUMMARY$ROCAUC <- as.numeric(as.character(CB.SUMMARY$ROCAUC))
CB.SUMMARY$ApparentSensitivity <- as.numeric(as.character(CB.SUMMARY$ApparentSensitivity))
CB.SUMMARY$ApparentSpecificity <- as.numeric(as.character(CB.SUMMARY$ApparentSpecificity))

(CB.Threshold <- CB.SUMMARY$OptimalThreshold.CB)

(CB.Sensitivity <- CB.SUMMARY$ApparentSensitivity)

(CB.Specificity <- CB.SUMMARY$ApparentSpecificity)
...
|
| **[E]** The dichotomization threshold criterion which supported a cost-benefit approach by considering the costs of false positive or negative cases by demonstrating the best and most consistent metrics is as follows:
|      **[E.1]** **CB** with Apparent Sensitivity = 0.84733, Apparent Specificity = 0.80796
|
|
...
ClassificationMetrics <- c(MINVALUESE.Sensitivity,
                           MINVALUESP.Sensitivity,
                           MAXPRODSPSE.Sensitivity,
                           ROC01.Sensitivity,
                           SPEQUALSE.Sensitivity,
                           YOUDEN.Sensitivity,
                           MAXEFFICIENCY.Sensitivity,
                           MINIMAX.Sensitivity,
                           MAXDOR.Sensitivity,
                           MAXKAPPA.Sensitivity,
                           MINVALUENPV.Sensitivity,
                           MINVALUEPPV.Sensitivity,
                           NPVEQUALPPV.Sensitivity,
                           MINPVALUE.Sensitivity,
                           CB.Sensitivity,
                           MINVALUESE.Specificity,
                           MINVALUESP.Specificity,
                           MAXPRODSPSE.Specificity,
                           ROC01.Specificity,
                           SPEQUALSE.Specificity,
                           YOUDEN.Specificity,
                           MAXEFFICIENCY.Specificity,
                           MINIMAX.Specificity,
                           MAXDOR.Specificity,
                           MAXKAPPA.Specificity,
                           MINVALUENPV.Specificity,
                           MINVALUEPPV.Specificity,
                           NPVEQUALPPV.Specificity,
                           MINPVALUE.Specificity,
                           CB.Specificity)
...
|
| ![](D:/Github_Codes/ProjectPortfolio/Portfolio_Project_24/images/Project24_Summary.png)
|
# **3. References**
|
...
| **[Publication]** [Primer on Certain Elements of Medical Decision Making](https://www.nejm.org/doi/full/10.1056/NEJM197507312930501) by Barbara McNeil, Emmett Keeler and James Adelstein (New England Journal of Medicine)
